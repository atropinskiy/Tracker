...
class MainViewController: UIViewController {
    ...
    private func addControlView() {
        ...
        dateButton.backgroundColor = UIColor(red: 240/255.0, green: 240/255.0, blue: 240/255.0, alpha: 1.0)
        dateButton.layer.cornerRadius = 8.0
        dateButton.setTitleColor(UIColor(named: "YP-black"), for: .normal)
        controlView.addSubview(dateButton)
        NSLayoutConstraint.activate([
            dateButton.centerYAnchor.constraint(equalTo: addButton.centerYAnchor),
            dateButton.widthAnchor.constraint(greaterThanOrEqualToConstant: 100),
            dateButton.trailingAnchor.constraint(equalTo: controlView.trailingAnchor, constant: -16),
    ...
    private func setupDatePicker() {
        ...
        datePicker.isHidden = true // Скрываем datePicker по умолчанию
        datePicker.translatesAutoresizingMaskIntoConstraints = false
        view.addSubview(datePicker)

        // Ограничения для datePicker
        NSLayoutConstraint.activate([
            datePicker.topAnchor.constraint(equalTo: dateButton.bottomAnchor, constant: 10), // Расположим ниже кнопки
    ...
    @objc func buttonTapped() {
        datePicker.isHidden.toggle() // Переключаем видимость UIDatePicker
    }
    
    @objc func datePickerChanged() {
        let dateFormatter = DateFormatter()
        dateFormatter.dateStyle = .medium
        dateButton.setTitle(dateFormatter.string(from: datePicker.date), for: .normal) // Обновляем текст кнопки
    }
}
